...
/* ── Print sheet (same function as QualityCheckPage for consistency) ──── */
...
function AddItemsMode({ user }) {
  ...
  return (
    <div className="space-y-4">
      <input ref={inputRef} className="sr-only" readOnly aria-hidden="true" tabIndex={-1} />
...
function PutawayMode() {
  ...
  const handleReassign = async (planId) => {
    try {
      ...
      toast({ title: "Reassigned", description: `New bin: ${res.new_bin_id}` });
      setResult(null); setLastScan({ value: "", type: "" });
    } catch (err) { toast({ title: "Error", description: err.message, variant: "destructive" }); }
  };
