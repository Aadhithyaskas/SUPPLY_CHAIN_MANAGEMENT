...
function ZoneAssignDialog({ product, zones, categories, onClose, onSaved }) {
  ...
  const filteredZones = suggestedZoneType
    ? zones.filter(z => z.zone_type === suggestedZoneType)
    : zones;
  ...
  const save = async (e) => {
    ...
    setSaving(true);
    try {
      await assignProductZone(product.product_id, form);
      toast({ title: "Zone Assigned", description: `${product.product_name} assigned to zone ${form.zone}.` });
      onSaved();
      onClose();
    } catch (err) {
      toast({ title: "Error", description: err.message, variant: "destructive" });
