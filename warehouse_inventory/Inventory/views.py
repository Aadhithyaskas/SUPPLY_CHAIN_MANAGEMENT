...
class ASNListView(APIView):
    ...
    def get(self, request):
        asns = ASN.objects.select_related("po", "vendor").all()
...
class ReassignPutawayBinView(APIView):
    ...
    def post(self, request, plan_id):
        ...
        try:
            new_bin = assign_bin(plan.product, plan.planned_quantity)
        except ValueError as exc:
            return Response({"error": str(exc)}, status=status.HTTP_400_BAD_REQUEST)

        if new_bin.bin_id == old_bin.bin_id:
            return Response(
                {
                    "error": (
                        "System re-assigned the same bin. "
                        "If the bin is physically full, mark it manually and retry."
                    )
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
